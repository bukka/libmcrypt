...
void GenPub(HWND hwndDlg)
{
    ...
    t_out = NULL;
    res = dll_make_key(0, t_key, 0, &t_out);
...
void Sig(HWND hwndDlg)
{
    ...
    GrayAll(hwndDlg, TRUE);
    t_out = NULL;
    res = dll_sign(0, t_key, 0, 0, 0, t_inp, &t_out);
    if (CheckError(hwndDlg, res))
...
void Ver(HWND hwndDlg)
{
    ...
    GrayAll(hwndDlg, TRUE);
    t_out = NULL;
    res = dll_verify(0, t_key, 0, 0, 0, 0, t_inp, &t_out, 0);
    if (CheckError(hwndDlg, res)) {
        SetDlgItemText(hwndDlg, IDC_TEXT, t_out);
...
void Enc(HWND hwndDlg)
{
    ...
    t_key[1] = malloc(KEYSIZE+2);
    if (!t_inp || !t_key[0] || !t_key[1]) {
        if(t_inp) free(t_inp);
        if(t_key[0]) free(t_key[0]);
        if(t_key[1]) free(t_key[1]);
        return;
    }
    GetDlgItemText(hwndDlg, IDC_TEXT, t_inp, s_inp);
    GetDlgItemText(hwndDlg, keysrc, t_key[0], s_key);
    if (!conv && toself && GetNumKeys() && GetKeyPtr(0)) {
        strncpy(t_key[1], GetKeyPtr(0), KEYSIZE);
        t_key[1][KEYSIZE] = 0;
    } else
        toself = 0;
    if (conv) {
    ...
    t_out = NULL;
    if (conv)
        res = dll_encrypt_c(0, t_key[0], 0, 0, 0, t_inp, &t_out);
    else
        res = dll_encrypt_pk(0, t_key, 0, 0, 0, 0, t_inp, &t_out, t_sec, toself?2:1);
    if (CheckError(hwndDlg, res))
        SetDlgItemText(hwndDlg, IDC_TEXT, t_out);
...
void Dec(HWND hwndDlg)
{
    ...
    t_out = NULL;
    if (conv)
        res = dll_decrypt_c(0, t_key, 0, 0, 0, t_inp, &t_out);
    else
        res = dll_decrypt_pk(0, t_key, 0, 0, 0, t_inp, &t_out, &keyn);
...
void SigFile(HWND hwndDlg)
{
    ...
    //    return;
    GrayAll(hwndDlg, TRUE);
    res = dll_sign(0, t_key, t_inp, t_out, binmode, 0, 0);
    memset(t_key,0,s_key);
    free(t_key);
    free(t_inp);
    free(t_out);
...
void VerFile(HWND hwndDlg)
{
    ...
    //    return;
    GrayAll(hwndDlg, TRUE);
    res = dll_verify(0, t_key, t_inp, t_out, t_sec, binmode, 0, 0, 0);
    free(t_key);
    free(t_inp);
    if (t_out) free(t_out);
    if (t_sec) free(t_sec);
...
void EncFile(HWND hwndDlg)
{
    ...
    t_key[1] = malloc(KEYSIZE+2);
    t_out = malloc(s_inp+6);
    if (!t_inp || !t_key[0] || !t_key[1] || !t_out) {
        if(t_inp) free(t_inp);
        if(t_key[0]) free(t_key[0]);
        if(t_key[1]) free(t_key[1]);
        if(t_out) free(t_out);
    ...
    GrayAll(hwndDlg, TRUE);
    if (conv)
        res = dll_encrypt_c(0, t_key[0], t_inp, t_out, binmode, 0, 0);
    else
        res = dll_encrypt_pk(0, t_key, t_inp, t_out, 0, binmode, 0, 0, t_sec, toself?2:1);
    prng_set_rnd(0,0,0,0); prng_set_rnd(0,0,0,0);
    memset(t_key[0],0,s_key);
    free(t_key[0]);
    free(t_key[1]);
...
void DecFile(HWND hwndDlg)
{
    ...
    conv = IsDlgButtonChecked(hwndDlg, IDC_CONV);
    if (conv)
        res = dll_decrypt_c(0, t_key, t_inp, t_out, binmode, 0, 0);
    else
        res = dll_decrypt_pk(0, t_key, t_inp, t_out, binmode, 0, 0, &keyn);
    memset(t_key,0,s_key);
    free(t_key);
    free(t_inp);
    free(t_out);
